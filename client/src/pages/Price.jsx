...
function Price() {
  ...
  // ናይ ኖትቡክ ስቴት (Customer Agreement Notebook)
  ...
  const [currentAgreement, setCurrentAgreement] = useState({
    ...
    selectedPackageKey: 'gold',
    customPrice: '',
    customNotes: '',
    // ኣብቲ ፓኬጅ ዘለዉ ኣገልግሎታት ንምእራም
    editableServices: [],
    editableFeatures: []
  });
  ...
  useEffect(() => {
    // ዳታ ካብ localStorage ን Agreement Notebook ንምዕቃብ
    const savedAgreements = localStorage.getItem('habeshaCustomerAgreements');
    if (savedAgreements) {
      try {
        setCustomerAgreements(JSON.parse(savedAgreements));
      } catch (e) {
        console.error("Error loading agreements", e);
      }
    }

    // ፈለማ እንትጅምር ነቲ ሓደ ምርጫ ናይ 'gold' ዋጋን ትሕዝቶን ኣብ ፎርም ክመልኦ ንገብሮ
    const initialPkg = defaultPackages['gold'];
    setCurrentAgreement(prev => ({
      ...prev,
      customPrice: initialPkg.price,
      editableServices: [...initialPkg.services],
      editableFeatures: [...initialPkg.features]
    }));
  ...
  // ፓኬጅ ምስ ተቐየረ፡ ነቲ ናይቲ ፓኬጅ ዋጋን ትሕዝቶን ብአውቶማቲክ ናብቲ ፎርም ከምጽኦ ንገብሮ (Edit Mode features included)
  const handlePackageChange = (e) => {
    const pkgKey = e.target.value;
    const pkg = packages[pkgKey] || packages.gold;
    setCurrentAgreement(prev => ({
      ...prev,
      selectedPackageKey: pkgKey,
      customPrice: pkg.price,
      editableServices: [...pkg.services],
      editableFeatures: [...pkg.features]
    }));
  };
  ...
  const handleSaveAgreement = (e) => {
    e.preventDefault();
    if (!currentAgreement.customerName.trim()) {
      alert("በጃኹም ስም ካስተመር ኣእትዉ!");
      return;
    }

    let updatedAgreements;
    if (currentAgreement.id !== null) {
      updatedAgreements = customerAgreements.map(item => 
        item.id === currentAgreement.id ? currentAgreement : item
      );
    } else {
      const newItem = { ...currentAgreement, id: Date.now() };
      updatedAgreements = [newItem, ...customerAgreements];
    }

    setCustomerAgreements(updatedAgreements);
    localStorage.setItem('habeshaCustomerAgreements', JSON.stringify(updatedAgreements));
    
    alert("ናይ ካስተመር ኖት ስምምዕ ብሰላም ተዓቚሩ ኣሎ! 📓");
  };

  // ካብቲ ዝተመዝገበ ሊስት ንምእራም
  ...
  const handleDeleteItem = (id) => {
    if (window.confirm(" ነዚ ኖት ክትደምስሶ ትደልዩ ኢኹም?")) {
      const updated = customerAgreements.filter(item => item.id !== id);
      setCustomerAgreements(updated);
      localStorage.setItem('habeshaCustomerAgreements', JSON.stringify(updated));
    }
  };

  // ናብ ዋትሳፕ ወይ ማሕበራዊ መራኸቢታት ንምልኣኽ (Share)
  const handleShare = (item) => {
    const pkg = packages[item.selectedPackageKey] || packages.gold;
    
    const servicesText = item.editableServices && item.editableServices.length > 0 
      ? item.editableServices.join('%0A') 
      : 'የሎን';

    const featuresText = item.editableFeatures && item.editableFeatures.length > 0 
      ? item.editableFeatures.join('%0A') 
      : 'የሎን';

    const text = `📋 *Habesha Film Production - Customer Agreement*%0A%0A` +
      `👤 ካስተመር: *${item.customerName}*%0A` +
      `📅 ዕለት: ${item.date}%0A` +
      `📦 ፓኬጅ: *${pkg.name} (${pkg.tier})*%0A` +
      `💰 ዋጋ: *${item.customPrice}*%0A%0A` +
      `🎬 *ናይ ቀረጻ ኣገልግሎታት:*%0A${servicesText}%0A%0A` +
      `🎁 *ናይ ስእምን ውህብቶን ትሕዝቶ (Features):*%0A${featuresText}%0A%0A` +
      `📝 *ተወሳኺ ርእይቶ / ሓባሪ:*%0A${item.customNotes || 'ዝኾነ ተወሳኺ ርእይቶ የሎን'}%0A%0A` +
      `ኣገልግሎትና ስለዝመረጽኩም እናመስገንና!`;
    ...
    <div className="min-h-screen bg-[#050505] text-zinc-100 font-sans selection:bg-[#dfb557]/30 selection:text-[#dfb557] overflow-x-hidden flex flex-col justify-between">
      ...
      <div className="flex-grow flex items-center justify-center px-4 py-32">
        {!isAuthenticated ? (
          <div className="bg-zinc-950 p-8 md:p-12 shadow-2xl border-2 border-[#dfb557]/40 rounded-2xl max-w-md w-full text-center relative">
            <span className="text-[9px] md:text-[10px] tracking-[0.4em] uppercase text-[#dfb557] font-semibold block mb-2">Secure Access</span>
            <h2 className="text-2xl md:text-3xl font-serif mb-3 text-zinc-100">Protected Notebook Page</h2>
            <div className="w-12 h-[1px] bg-[#dfb557]/40 mx-auto mb-4"></div>
            <p className="text-xs md:text-sm text-zinc-400 mb-6 font-light">እዚ ገጽ ብሚጢራዊ ፓስኮድ ዝተዓጸወ እዩ። በጃኹም ፓስኮድ ኣእትዉ።</p>
          ...
          /* Notebook Generator Workspace */
          <div className="max-w-4xl mx-auto px-4 py-8 w-full space-y-8">
            <div className="text-center">
              <span className="text-[10px] md:text-[11px] tracking-[0.5em] uppercase text-[#dfb557] font-medium block mb-2">Admin Dashboard</span>
              <h1 className="text-3xl font-serif mb-2 text-zinc-100">Customer Agreement Notebook 📓</h1>
              <div className="w-12 h-[1px] bg-[#dfb557]/40 mx-auto mb-4"></div>
              <p className="text-xs text-zinc-400">ስም ካስተመር፡ ዕለት ኣእትዩ፡ ካብቶም ፓኬታት ሓደ መሪጽኩም ክትእርሙን ክትልእኩን ትኽእሉ።</p>
            </div>

            {/* Main Notebook Form */}
            <div className="bg-zinc-950 border border-[#dfb557]/40 p-6 md:p-8 rounded-2xl shadow-2xl space-y-6">
              <h3 className="text-sm font-bold uppercase text-[#dfb557]">
                {currentAgreement.id !== null ? '✏️ Edit Customer Agreement Note' : '➕ Create New Agreement Note'}
              </h3>

              <form onSubmit={handleSaveAgreement} className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="text-[10px] uppercase text-zinc-400 font-semibold block mb-1">ስም ካስተመር (Customer Name)</label>
                    <input 
                      type="text"
                      placeholder="ንኣብነት: ዮናስ ተስፋይ"
                      value={currentAgreement.customerName}
                      onChange={(e) => setCurrentAgreement({...currentAgreement, customerName: e.target.value})}
                      className="w-full bg-zinc-900 border border-zinc-700 p-3 rounded-xl text-xs text-zinc-100"
                      required
                    />
                  </div>

                  <div>
                    <label className="text-[10px] uppercase text-zinc-400 font-semibold block mb-1">ዕለት (Date)</label>
                    <input 
                      type="date"
                      value={currentAgreement.date}
                      onChange={(e) => setCurrentAgreement({...currentAgreement, date: e.target.value})}
                      className="w-full bg-zinc-900 border border-zinc-700 p-3 rounded-xl text-xs text-zinc-100"
                    />
                  </div>

                  <div>
                    <label className="text-[10px] uppercase text-zinc-400 font-semibold block mb-1">መማረጺ ፓኬጅ (Select Package)</label>
                    <select 
                      value={currentAgreement.selectedPackageKey}
                      onChange={handlePackageChange}
                      className="w-full bg-zinc-900 border border-[#dfb557]/50 p-3 rounded-xl text-xs text-[#dfb557] font-bold"
                    >
                ...
                <div>
                  <label className="text-[10px] uppercase text-zinc-400 font-semibold block mb-1">ዋጋ (Price - ካብቲ ኤዲት ሙድ ዝመጸ)</label>
                  <input 
                    type="text"
                    value={currentAgreement.customPrice}
                    onChange={(e) => setCurrentAgreement({...currentAgreement, customPrice: e.target.value})}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] uppercase text-zinc-400 font-semibold block mb-1">ናይ ቀረጻ ኣገልግሎታት (Services - ነንሰርሓ እዩ)</label>
                    <textarea 
                      rows="4"
                      value={currentAgreement.editableServices.join('\n')}
                      onChange={(e) => setCurrentAgreement({...currentAgreement, editableServices: e.target.value.split('\n')})}
                      className="w-full bg-zinc-900 border border-zinc-700 p-3 rounded-xl text-xs text-zinc-200"
                    />
                  </div>

                  <div>
                    <label className="text-[10px] uppercase text-zinc-400 font-semibold block mb-1">ናይ ስእምን ውህብቶን ትሕዝቶ (Features)</label>
                    <textarea 
                      rows="4"
                      value={currentAgreement.editableFeatures.join('\n')}
                      onChange={(e) => setCurrentAgreement({...currentAgreement, editableFeatures: e.target.value.split('\n')})}
                      className="w-full bg-zinc-900 border border-zinc-700 p-3 rounded-xl text-xs text-zinc-200"
                    />
                  </div>
                </div>
                ...
                <div>
                  <label className="text-[10px] uppercase text-zinc-400 font-semibold block mb-1">ተወሳኺ ርእይቶ / ሓባሪ (Custom Note)</label>
                  <textarea 
                    rows="2"
                    placeholder="ንኣብነት: ዋጋ 20,000 ቅነሳ ተገይሩሉ..."
                    value={currentAgreement.customNotes}
                    onChange={(e) => setCurrentAgreement({...currentAgreement, customNotes: e.target.value})}
                    className="w-full bg-zinc-900 border border-zinc-700 p-3 rounded-xl text-xs text-zinc-200"
                  />
                </div>
                ...
                <div className="flex justify-end gap-3 pt-2">
                  {currentAgreement.id !== null && (
                    <button 
                      type="button"
                      onClick={() => {
                        const def = defaultPackages['gold'];
                        setCurrentAgreement({
                          id: null,
                          customerName: '',
                          date: new Date().toISOString().split('T')[0],
                          ...
                          customNotes: '',
                          editableServices: [...def.services],
                          editableFeatures: [...def.features]
                        });
                      }}
                      className="px-4 py-2 bg-zinc-800 text-zinc-300 rounded-xl text-xs uppercase font-bold"
                    >
                      Cancel Edit
                    </button>
                  )}
                  <button type="submit" className="px-6 py-3 bg-[#dfb557] text-black rounded-xl text-xs uppercase font-bold tracking-widest hover:bg-[#c99f45] shadow-lg">
                    {currentAgreement.id !== null ? 'Update Note 📝' : 'Save to Notebook 💾'}
                  </button>
                </div>
              </form>
            </div>
            ...
            <div className="bg-zinc-950 border border-zinc-800 p-6 md:p-8 rounded-2xl space-y-4">
              <h4 className="text-xs uppercase font-bold tracking-widest text-[#dfb557]">Saved Agreements History ({customerAgreements.length})</h4>
              
              {customerAgreements.length === 0 ? (
                <p className="text-zinc-500 text-xs italic">ክሳብ ሕጂ ዝተመዝገበ ኖት የሎም።</p>
              ) : (
                <div className="space-y-4">
                  {customerAgreements.map((item) => {
                    const pkg = packages[item.selectedPackageKey] || packages.gold;
                    return (
                      <div key={item.id} className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div className="space-y-1">
                          <div className="flex items-center gap-3">
                            <span className="text-sm font-bold text-zinc-100">{item.customerName}</span>
                            <span className="text-[10px] bg-[#dfb557]/20 text-[#dfb557] px-2 py-0.5 rounded-full font-semibold">{pkg.name}</span>
                            <span className="text-xs text-zinc-400">📅 {item.date}</span>
                          </div>
                          <p className="text-xs text-zinc-300">
                            ዋጋ: <span className="text-[#dfb557] font-bold">{item.customPrice}</span>
                          </p>
                          {item.customNotes && (
                            <p className="text-xs text-zinc-400 italic">📝 ኖት: {item.customNotes}</p>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-2 w-full md:w-auto justify-end">
                          <button 
                            ...
                            className="px-3 py-1.5 bg-green-600 hover:bg-green-500 text-white rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 shadow-md"
                          >
                            💬 Share WhatsApp
                          </button>
                          <button 
                            onClick={() => handleEditItem(item)} 
                            className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 rounded-lg text-[10px] font-bold uppercase tracking-wider"
                          >
                            Edit ✏️
                          </button>
                          <button 
                            onClick={() => handleDeleteItem(item.id)} 
                            className="px-3 py-1.5 bg-red-950/60 hover:bg-red-900 text-red-300 rounded-lg text-[10px] font-bold uppercase tracking-wider"
                          >
                            Delete 🗑️
                          </button>
                        </div>
