...
function Price() {
  ...
  // ካብ ኣድሚን ዝመጽእ ናይ ፕራይሲንግ ሓበሬታ ንምዕቃብ
  ...
  useEffect(() => {
    ...
    fetch('https://habesha-film-production-server.onrender.com/api/projects')
      ...
      .then(data => {
        ...
        if (pricingItem) {
          setPricingData({
            desc: pricingItem.desc || pricingItem.description || '',
            names: pricingItem.names || ''
          });
        }
      ...
      <div className="flex-grow flex items-center justify-center px-4 py-32">
        {!isAuthenticated ? (
          ...
          <div className="max-w-7xl mx-auto text-center px-4 py-12 w-full">
            ...
            {/* ካብ ኣድሚን ዝመጸ ዳታ እንተሎ ኣብዚ ብዲናሚክ ይርአ */}
            {dataLoading ? (
              <p className="text-zinc-500 py-12">Loading pricing packages...</p>
            ) : pricingData.desc ? (
              <div className="bg-zinc-950/90 border-2 border-[#dfb557]/50 p-8 rounded-2xl max-w-4xl mx-auto text-left shadow-2xl">
                <span className="text-[10px] uppercase font-bold tracking-[0.3em] text-[#dfb557] block mb-2">Admin Managed Package Details</span>
                <div className="whitespace-pre-line text-zinc-200 font-light text-sm md:text-base leading-relaxed">
                  {pricingData.desc}
                </div>
              </div>
            ) : (
              /* ኣድሚን ገና ሓበሬታ ካብ ዘይሰፈረሉ ነቲ ናይ ቀደም ስታንዳርድ ፓኬኬት ብዲፎልት የርእዮ */
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-left">
