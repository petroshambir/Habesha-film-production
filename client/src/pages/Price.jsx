...
function Price() {
  ...
  // 1. መጀመርታ pricingData ንኹሉ ፓኬጅ ዝሓዘ ክኸውን ኣለዎ
  ...
  useEffect(() => {
    ...
    // ካብ ዳታቤዝ ናይ Pricing ሓበሬታ ከተሓትት
    fetch('https://habesha-film-production-server.onrender.com/api/projects')
      .then(res => res.json())
      .then(data => {
        const pricingItem = data.find(item => item.title === 'Pricing');
        if (pricingItem && pricingItem.desc) {
          try {
            // እቲ desc ናብ ጃቫስክሪፕት ኦብጀክት ንምልዋጥ
            const parsedTiers = typeof pricingItem.desc === 'string' ? JSON.parse(pricingItem.desc) : pricingItem.desc;
            setPricingData({
      ...
      <div className="flex-grow flex items-center justify-center px-4 py-32">
        {!isAuthenticated ? (
          ...
          <div className="max-w-7xl mx-auto text-center px-4 py-12 w-full">
            ...
            {/* ካብ ኣድሚን ዝመጸ ዳታ ብዲናሚክ ንምርኣይ */}
            {dataLoading ? (
              <p className="text-zinc-500 py-12">Loading pricing packages...</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-left">
                {pricingData.pricingTiers && Object.entries(pricingData.pricingTiers).map(([tier, info]) => (
                  <div key={tier} className="bg-zinc-950 border-2 border-[#dfb557]/50 p-6 sm:p-8 rounded-2xl shadow-2xl flex flex-col justify-between transition-transform hover:-translate-y-1">
                    <div>
                      <span className="text-[10px] uppercase font-bold tracking-[0.3em] text-[#dfb557]">{tier} Package</span>
                      <h3 className="text-2xl font-serif mt-1 mb-2 text-zinc-100">{tier}</h3>
                      <p className="text-3xl font-serif font-bold text-[#dfb557] mb-6">{info.price || 'Contact Us'}</p>
                      
                      <ul className="text-xs sm:text-sm text-zinc-300 space-y-2 mb-8 font-light">
                        {info.details ? (
                          info.details.split(',').map((item, i) => (
                            <li key={i} className="flex items-center gap-2">✓ {item.trim()}</li>
                          ))
                        ) : (
                          <li className="text-zinc-500">ዝርዝር ሓበሬታ የብሉን</li>
                        )}
                      </ul>
                    </div>
                    <button className="w-full bg-zinc-900 border border-[#dfb557]/50 text-zinc-100 py-3 text-[11px] uppercase font-bold tracking-[0.3em] hover:bg-[#dfb557] hover:text-black transition-all duration-300 rounded-xl shadow-md">
