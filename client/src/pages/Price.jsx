...
function Price() {
  ...
  const handleShareReceipt = (note) => {
    // 💡 ፉተር (Receipt): ስም፡ ዕለት፡ ዋጋ ከምኡውን ናይ ቀረጻ ኣገልግሎታት (Services) ዝሓዘ
    const receiptText = `🎥 ═══════════════════════════ 🎥\n` +
      `   HABESHA FILM PRODUCTION\n` +
      `   Professional Media & Wedding Cinematography\n` +
      `🎥 ═══════════════════════════ 🎥\n\n` +
      ...
      `💰 **ዋጋ (Total Price):** ${note.packagePrice}\n` +
      `----------------------------------------\n` +
      `📌 **ናይ ቀረጻ ኣገልግሎታት (Services):**\n` +
      (note.packageServices && note.packageServices.length > 0 
        ? note.packageServices.map(s => `• ${s}`).join('\n') + '\n' 
        : '• ዝኾነ ፍሉይ ኣገልግሎት ኣይተሓተመን\n') + `\n` +
      `═══════════════════════════════\n` +
      `🌐 Habesha Film Production\n` +
      `📍 Professional Studio & Media Production\n` +
      `✨ መጻኢ ፕሮጀክትታትኩም ብሉጽ ብዝኾነ ኣገባብ ነሰርሕ! እናመስገንና።`;
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-100 font-sans selection:bg-[#dfb557]/30 selection:text-[#dfb557] overflow-x-hidden flex flex-col justify-between">
          ...
          <div className="max-w-7xl mx-auto text-center px-4 py-12 w-full">
            ...
            <div className="bg-zinc-950 border border-[#dfb557]/40 p-6 md:p-8 rounded-2xl space-y-8 text-left shadow-2xl">
              ...
              <div className="bg-zinc-900 p-6 rounded-xl border border-[#dfb557]/30 space-y-4 shadow-inner">
                <div className="flex justify-between items-center border-b border-zinc-800 pb-3">
                  <h3 className="text-xs font-bold uppercase text-[#dfb557] tracking-wider">📝 Admin Notebook & Customer Bookings</h3>
                  <span className="text-[10px] text-zinc-400 font-light">ስም፡ ዕለት፡ ዋጋን ሰርቪስን ዝሓዘ</span>
                </div>

                <div className="space-y-4 pt-2 max-h-[500px] overflow-y-auto">
                  {notebookList.length === 0 ? (
                    <p className="text-zinc-500 text-xs italic text-center py-4">ዝኾነ ዝተመዝገበ ዓሚል ወይ ኖት የልቦን።</p>
                  ) : (
                    notebookList.map((note) => (
                      <div key={note.id} className="bg-zinc-950 border border-zinc-800 p-5 rounded-xl space-y-4 shadow-md">
                        ...
                        <div className="bg-zinc-900/80 border border-[#dfb557]/30 p-4 rounded-xl space-y-3">
                          ...
                          {/* Services Displayed Inside Notebook */}
                          {note.packageServices && note.packageServices.length > 0 && (
                            <div className="space-y-1 pt-2 border-t border-zinc-800 text-xs">
                              <span className="text-[10px] text-[#dfb557] font-semibold uppercase block">ናይ ቀረጻ ኣገልግሎታት (Services):</span>
                              <ul className="space-y-1 text-zinc-300 font-light">
                                {note.packageServices.map((s, i) => <li key={i}>{s}</li>)}
                              </ul>
                            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-left">
              ...
              <div className="bg-zinc-950/90 border-2 border-[#dfb557]/50 p-6 sm:p-8 rounded-2xl shadow-2xl flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-3xl font-serif font-bold text-[#dfb557] mb-6">{packages.premium.price}</p>
                  <p className="text-xs sm:text-sm text-zinc-300 mb-6 font-light leading-relaxed">{packages.premium.desc}</p>
                  <button onClick={() => handleSelectPackageClick('premium')} className="w-full bg-[#dfb557] text-black py-3 uppercase text-xs font-bold tracking-[0.2em] hover:bg-[#c99f45] transition-all rounded-xl mt-4">Select Package ➔</button>
                </div>
              </div>
              ...
              <div className="bg-zinc-950 border-2 border-[#dfb557] p-6 sm:p-8 rounded-2xl shadow-2xl relative flex flex-col justify-between">
                ...
                <div>
                  ...
                  {packages.gold.services && packages.gold.services.length > 0 && (
                    <div className="text-xs sm:text-sm text-zinc-300 mb-4 font-light space-y-1.5 border-b border-zinc-800 pb-3">
                      <p className="text-[#dfb557] font-semibold uppercase tracking-wider text-[10px]">ናይ ቀረጻ ኣገልግሎታት (Services):</p>
                      <ul className="space-y-1 pl-1">
                        {packages.gold.services.map((srv, idx) => (
                          <li key={idx}>{srv}</li>
                        ))}
                  ...
                  <button onClick={() => handleSelectPackageClick('gold')} className="w-full bg-[#dfb557] text-black py-3 uppercase text-xs font-bold tracking-[0.2em] hover:bg-[#c99f45] transition-all rounded-xl mt-4">Select Package ➔</button>
                </div>
              </div>
              ...
              <div className="bg-zinc-950/90 border-2 border-[#dfb557]/50 p-6 sm:p-8 rounded-2xl shadow-2xl flex flex-col justify-between">
                <div>
                  ...
                  <button onClick={() => handleSelectPackageClick('silver')} className="w-full bg-[#dfb557] text-black py-3 uppercase text-xs font-bold tracking-[0.2em] hover:bg-[#c99f45] transition-all rounded-xl mt-4">Select Package ➔</button>
                </div>
              </div>
              ...
              <div className="bg-zinc-950/70 border-2 border-[#dfb557]/30 p-6 sm:p-8 rounded-2xl shadow-xl flex flex-col justify-between">
                <div>
                  ...
                  <button onClick={() => handleSelectPackageClick('standard')} className="w-full bg-[#dfb557] text-black py-3 uppercase text-xs font-bold tracking-[0.2em] hover:bg-[#c99f45] transition-all rounded-xl mt-4">Select Package ➔</button>
                </div>
              </div>
      ...
      {/* --- Booking Modal / Form Dialog --- */}
      {isBookingModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-950 border border-[#dfb557]/50 rounded-2xl p-6 sm:p-8 max-w-lg w-full shadow-2xl relative space-y-6">
            <div className="flex justify-between items-center border-b border-zinc-800 pb-4">
              <div>
                <span className="text-[10px] uppercase font-bold tracking-[0.3em] text-[#dfb557]">{selectedPackage?.tier}</span>
                <h3 className="text-xl font-serif text-white">{selectedPackage?.name} Booking Form</h3>
              </div>
              <button 
                onClick={() => setIsBookingModalOpen(false)}
            ...
            <form onSubmit={handleBookingSubmit} className="space-y-4">
              <div>
                ...
                <input 
                  type="text"
                  required
                  placeholder="e.g. Dawit & Helen"
                  value={customerName}
                  onChange={(e) => setCustomerName(e.target.value)}
                  className="w-full bg-zinc-900 border border-zinc-700 p-3 rounded-xl text-sm text-white focus:outline-none focus:border-[#dfb557]"
                />
              </div>
              ...
              <div>
                <label className="text-[10px] uppercase text-zinc-400 font-semibold block mb-1">ዋጋ (Price) - ክትእርሞ ትኽእል ኢኻ</label>
                <input 
                  type="text"
                  required
                  value={customizedPrice}
                  onChange={(e) => setCustomizedPrice(e.target.value)}
                  className="w-full bg-zinc-900 border border-zinc-700 p-3 rounded-xl text-sm text-[#dfb557] font-bold focus:outline-none focus:border-[#dfb557]"
                />
              </div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-zinc-900">
                <button 
                  type="button"
                  onClick={() => setIsBookingModalOpen(false)}
                  className="px-5 py-2.5 bg-zinc-900 text-zinc-300 rounded-xl text-xs uppercase font-bold hover:bg-zinc-800 transition-all"
                >
                  Cancel
                </button>
                <button 
                  type="submit"
                  className="px-5 py-2.5 bg-[#dfb557] text-black rounded-xl text-xs uppercase font-bold hover:bg-[#c99f45] transition-all shadow-md"
                >
                  {editingNoteId !== null ? 'Update Record' : 'Save to Notebook'}
                </button>
              </div>
            </form>
          </div>
