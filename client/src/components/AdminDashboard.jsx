...
function SectionRenderer({ title, data, setData, onSave }) {
      ...
      fetch(`https://habesha-film-production-server.onrender.com/api/projects/${title}`)
        ...
        .then(item => {
          if (item) {
            ...
            try {
              parsedPricingTiers = typeof item.desc === 'string' ? JSON.parse(item.desc) : (item.desc || {});
            } catch (e) {
              parsedPricingTiers = { Standard: { price: '', details: item.desc || '' } };
            }
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      setData({ ...data, images: updatedImages, headings: updatedHeadings, descriptions: updatedDescriptions });
      alert(`${newImagesFromBackend.length} ስእሊ(ታት) ተሰቒሎም ኣለዉ!`);
    } catch (err) {
      console.error("Upload Error:", err);
      alert("ስእሊ ክስቀል ኣይከኣለን!");
  ...
  const deleteImage = (imgIndex) => {
    setData({
      ...data,
      images: (data.images || []).filter((_, i) => i !== imgIndex),
      headings: (data.headings || []).filter((_, i) => i !== imgIndex),
      descriptions: (data.descriptions || []).filter((_, i) => i !== imgIndex)
    });
  };
  ...
  return (
    <div className="p-4 md:p-8 border border-zinc-700 rounded-2xl bg-zinc-900 shadow-2xl overflow-hidden">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 border-b border-zinc-700 pb-4 gap-4">
        <h2 className="text-xl md:text-3xl font-bold text-amber-300">{title} Control Panel</h2>
        <button onClick={onSave} className="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg font-bold w-full sm:w-auto text-sm">Save {title}</button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="flex flex-col w-full md:col-span-2">
          {!isPricing && (
            <>
              <label className="block text-zinc-400 mb-2 text-sm">Names / Title:</label>
              <input type="text" value={data.names || ''} onChange={(e) => setData({ ...data, names: e.target.value })} className="bg-zinc-800 border border-zinc-600 p-3 rounded-lg w-full text-white mb-6 text-sm" placeholder="ማእከላይ ሽም" />
            </>
          )}

          <label className="block text-zinc-400 mb-2 text-sm">{isPricing ? 'Pricing Packages:' : 'Section Description:'}</label>
          {isPricing ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
              {['Standard', 'Silver', 'Gold', 'Premium'].map((tier) => (
                <div key={tier} className="bg-zinc-800 p-4 rounded-xl border border-zinc-700">
                  <h4 className="text-amber-400 font-bold mb-2 uppercase text-sm tracking-wider">{tier}</h4>
                  <input placeholder="Price" value={data.pricingTiers?.[tier]?.price || ''} onChange={(e) => {
                    const updatedTiers = { ...(data.pricingTiers || {}), [tier]: { ...(data.pricingTiers?.[tier] || {}), price: e.target.value } };
                    setData({ ...data, pricingTiers: updatedTiers, desc: JSON.stringify(updatedTiers) });
                  }} className="bg-zinc-900 border border-zinc-700 p-2 rounded w-full mb-3 text-xs text-white" />
                  <textarea placeholder="Details" value={data.pricingTiers?.[tier]?.details || ''} onChange={(e) => {
                    const updatedTiers = { ...(data.pricingTiers || {}), [tier]: { ...(data.pricingTiers?.[tier] || {}), details: e.target.value } };
                    setData({ ...data, pricingTiers: updatedTiers, desc: JSON.stringify(updatedTiers) });
                  }} className="bg-zinc-900 border border-zinc-700 p-2 rounded w-full h-24 text-xs text-white resize-none" />
                </div>
              ))}
            </div>
          ) : (
            <textarea rows="3" value={data.desc || ''} onChange={(e) => setData({ ...data, desc: e.target.value })} className="bg-zinc-800 border border-zinc-600 p-3 rounded-lg w-full text-white text-sm" />
          )}
        </div>
      </div>
      {/* ... additional UI for image management ... */}
    </div>
  );
}
...
function AdminDashboard() {
  ...
  // ... fetch logic, handleCreatePortal, handleClientImageUpload, handleDeletePortal, handleSave remain as provided ...

  return (
    <div className="bg-zinc-950 min-h-screen text-white flex flex-col md:flex-row relative">
      {/* Sidebar ... */}
      <aside className="w-full md:w-72 bg-zinc-900 border-r border-zinc-800 p-6">
        {/* Navigation links ... */}
      </aside>
        ...
        {sectionsConfig.map((sec) => {
          ...
          return (
            <SectionRenderer 
              ...
              data={currentData} 
              setData={(newData) => setSectionsData({ ...sectionsData, [sec.title]: newData })} 
              onSave={() => {/* Add handleSave call here */}} 
            />
          );
        })}
      </main>
