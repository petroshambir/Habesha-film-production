...
function AdminDashboard() {
  ...
  // ንዋጋታት (Prices) ዝምልከት ስቴት
  const [pricesList, setPricesList] = useState([
    { id: 'premium', name: 'Premium', price: '$2,000+', desc: 'ዝለዓለ ደረጃ ሞያዊ ክእለትን ብርክት ዝበሉ መሳርሒታትን ተጠቒምካ ዝስራሕ ቪአይፒ ኣገልግሎት።' },
    { id: 'gold', name: 'Gold', price: '300,000', desc: 'Exclusive top tier package' },
    { id: 'silver', name: 'Silver', price: '240,000', desc: 'Advanced package' },
    { id: 'standard', name: 'Standard', price: '190,000', desc: 'Standard package' }
  ]);

  // ናይ ሳድባር ንጡፍ ክፋል ንምምራጽ (Active Tab State)
  const [activeTab, setActiveTab] = useState('manager'); // 'manager', 'portal', 'prices', or section title
  ...
  const handlePriceChange = (id, field, value) => {
    setPricesList(prev => prev.map(item => item.id === id ? { ...item, [field]: value } : item));
  };
  ...
  const handleSavePrices = async () => {
    try {
      const res = await fetch('https://habesha-film-production-server.onrender.com/api/prices', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(pricesList)
      });
      if (res.ok) {
        alert(' ዋጋታት ብሰላም ተዓቂቦም ኣለዉ!');
      } else {
        alert(' ዋጋታት ምዕቃብ ኣይከኣለን። (Backend endpoint ክህሉ ኣለዎ)');
      }
    } catch (err) {
        ...
        {activeTab === 'manager' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              ...
              <div onClick={() => setActiveTab('prices')} className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl cursor-pointer hover:border-amber-500/50 transition-all group">
                <h3 className="text-lg font-bold text-amber-300 group-hover:text-amber-400 mb-1">Pricing Packages</h3>
                <p className="text-2xl font-black text-white mt-2">{pricesList.length} Tiers</p>
                <p className="text-xs text-zinc-500 mt-2">ናይ ፕሪሚየም፣ ጎልድ፣ ሲልቨርን ስታንዳርድ ዋጋታት ኣስተኻኽል</p>
              </div>
          ...
          <div className="space-y-6">
            <div className="p-4 md:p-6 border border-amber-500/50 rounded-2xl bg-zinc-900 shadow-2xl">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 border-b border-zinc-800 pb-4 gap-4">
                <div>
                  <h2 className="text-xl md:text-2xl font-bold text-amber-400">Manage Pricing Packages</h2>
                  <p className="text-xs text-zinc-400 mt-1">ኣብ price.jsk ዝረኣዩ ዋጋታትን መግለጫታትን ካብዚ ብቐጥታ ኣስተኻኽል</p>
                </div>
                <button 
                  onClick={handleSavePrices} 
                  className="bg-green-600 hover:bg-green-700 text-white px-6 py-2.5 rounded-lg font-bold w-full sm:w-auto text-sm shadow-lg transition-colors"
                >
                  Save Prices Changes
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {pricesList.map((pkg) => (
                  <div key={pkg.id} className="bg-zinc-800/60 border border-zinc-700 p-5 rounded-xl space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-xs uppercase tracking-widest text-amber-400 font-bold">{pkg.name} Package</span>
                      <span className="text-[10px] bg-zinc-900 px-2 py-1 rounded text-zinc-400 font-mono">ID: {pkg.id}</span>
                    </div>

                    <div>
                      <label className="block text-xs text-zinc-400 mb-1">Price Value (ዋጋ):</label>
                      <input 
                        type="text"
                        value={pkg.price || ''}
                        onChange={(e) => handlePriceChange(pkg.id, 'price', e.target.value)}
                        className="bg-zinc-900 border border-zinc-700 p-2.5 rounded-lg w-full text-white text-sm font-semibold text-amber-300"
                        placeholder="ንኣብነት: $2,000+ ወይ 300,000"
                      />
                    </div>

                    <div>
                      <label className="block text-xs text-zinc-400 mb-1">Description / Subtitle (መግለጫ):</label>
                      <textarea 
                        rows="2"
                        value={pkg.desc || ''}
                        onChange={(e) => handlePriceChange(pkg.id, 'desc', e.target.value)}
                        className="bg-zinc-900 border border-zinc-700 p-2.5 rounded-lg w-full text-white text-xs"
                        placeholder="ናይቲ ፓኬጅ ሓጺር መግለጫ..."
                      />
                    </div>
                  </div>
                ))}
