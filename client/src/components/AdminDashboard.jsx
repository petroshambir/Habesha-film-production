...
function AdminDashboard() {
  ...
  // New Passcode Generator State inside Admin
  const [newGeneratedPasscode, setNewGeneratedPasscode] = useState('');
  
  // Editable Price Packages State
  const [pricePackages, setPricePackages] = useState([
    {
      ...
      features: ['ዘይተወሰነ ሰዓታት ቀረጻ (Unlimited)', 'ክልተ ኤክስፐርት ካሜራማን', 'Cinematic Color Grading & VFX', '🎁 ቦናስ: ምሉእ ድሮን ቀረጻ + ሓደ ነጻ ዌብሳይት ባነር']
    },
    {
      id: 'gold',
      name: 'Gold',
      subtitle: 'Exclusive',
      price: '300,000',
      desc: 'ንዓበይቲ መርዓታ ዝኸውን ምሉእ መደብ።',
      features: ['ስቱዲዮ / ኣብ መስክ (2 ካሜራ)', 'ቃል ኪዳን (2 ካሜራ)', 'መዓልቲ መርዓ (5 ካሜራ)', '800 ፎቶዎች (10×15)', '2 ላሚኔትድ ፎቶ (30×90 & 30×60)']
    },
    {
      id: 'silver',
      name: 'Silver',
      subtitle: 'Advanced',
      price: '240,000',
      desc: 'ጽሩይን ምሹእን ማእከላይ ፓኬጅ።',
      features: ['ስቱዲዮ / ኣብ መስክ (2 ካሜራ)', 'ቃል ኪዳን (2 ካሜራ)', 'መዓልቲ መርዓ (4 ካሜራ)', '500 ፎቶዎች (10×15)']
    },
    {
      id: 'standard',
      name: 'Standard',
      subtitle: 'Standard',
      price: '190,000',
      desc: 'ቀሊልን ጠቃሚን መደብ ቀረጻ።',
      features: ['ስቱዲዮ / ኣብ መስክ (1 ቪድዮ፣ 1 ፎቶ)', 'ቃል ኪዳን (2 ካሜራ)', 'መዓልቲ መርዓ (3 ካሜራ)', '300 ፎቶዎች (10×15)']
    }
  ]);

  useEffect(() => {
    // 1. Fetch Portfolio Sections
    fetch('https://habesha-film-production-server.onrender.com/api/projects')
      .then(res => res.json())
      .then(data => {
        const dataMap = {};
    ...
    // 3. Fetch Prices from Database (if available)
    fetch('https://habesha-film-production-server.onrender.com/api/prices')
      ...
      .catch(err => console.log("Using default local prices"));
  ...
  // Passcode Generator Function inside Admin
  const generateNewPasscode = () => {
    ...
    setNewGeneratedPasscode(result);
  };

  const handleSavePricesToBackend = async () => {
    try {
      const res = await fetch('https://habesha-film-production-server.onrender.com/api/prices', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(pricePackages)
      });
      if (res.ok) {
        alert('ናይ ዋጋ ለውጥታት ብሰላም ተዓቂቦም! ኣብ Price.jsx ብኣውቶማቲክ ክልወጡ እዮም።');
      } else {
        alert(' ዋጋታት ከዕግብ ኣይከኣለን። (ሰርቨር ቼክ ግበር)');
      }
    } catch (err) {
  ...
  const handlePriceLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setIsPriceAuthenticated(true);
        const authData = { value: 'true', expiry: new Date().getTime() + (10 * 60 * 1000) };
        localStorage.setItem('priceAuthData', JSON.stringify(authData));
      } else {
        setError(true);
      }
      ...
      <aside className="w-full md:w-72 bg-zinc-900 border-b md:border-r border-zinc-800 p-4 md:p-6 flex flex-col justify-between shrink-0 md:sticky md:top-0 md:h-screen z-20">
        <div>
          ...
          <nav className="flex md:flex-col gap-2 overflow-x-auto pb-2 md:pb-0">
            <button onClick={() => setActiveTab('manager')} className={`px-4 py-3 rounded-xl font-medium text-sm transition-all text-left ${activeTab === 'manager' ? 'bg-amber-500 text-black font-bold' : 'text-zinc-300 hover:bg-zinc-800'}`}>
              📊 Dashboard Overview
            </button>
            <button onClick={() => setActiveTab('portal')} className={`px-4 py-3 rounded-xl font-medium text-sm transition-all text-left ${activeTab === 'portal' ? 'bg-amber-500 text-black font-bold' : 'text-zinc-300 hover:bg-zinc-800'}`}>
              👥 Client Portals ({portalsList.length})
            </button>
            <button onClick={() => setActiveTab('price')} className={`px-4 py-3 rounded-xl font-medium text-sm transition-all text-left ${activeTab === 'price' ? 'bg-amber-500 text-black font-bold' : 'text-zinc-300 hover:bg-zinc-800'}`}>
              💰 Price & Passcode Manager
            </button>
            {sectionsConfig.map((sec) => (
              <button key={sec.title} onClick={() => setActiveTab(sec.title)} className={`px-4 py-3 rounded-xl font-medium text-sm transition-all text-left ${activeTab === sec.title ? 'bg-amber-500 text-black font-bold' : 'text-zinc-300 hover:bg-zinc-800'}`}>
                ✨ {sec.title}
              </button>
            ))}
          </nav>
        ...
        {activeTab === 'manager' && (
          <div className="space-y-6">
            <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl">
              <h2 className="text-2xl font-bold text-amber-400 mb-2">Admin Dashboard</h2>
              <p className="text-zinc-400 text-sm">ካብዚ ሳድባር ኮይንካ ዋጋታት ክትቅይርን ፖርታላት ክተማሕድርን ትኽእል።</p>
            </div>
          </div>
        )}

        {/* Price & Passcode Management Tab */}
        {activeTab === 'price' && (
          <div className="space-y-8 max-w-5xl mx-auto">
            
            {/* Passcode Generator Section */}
            <div className="bg-zinc-900 border border-amber-500/40 p-6 rounded-2xl shadow-xl">
              <h3 className="text-xl font-bold text-amber-400 mb-2">🔑 Generate Admin Passcode</h3>
              <p className="text-xs text-zinc-400 mb-4">ን Price Page መእተዊ ሓድሽ ፓስኮድ ኣብዚ ክትፈጥር ትኽእል።</p>
              
              <div className="flex items-center gap-4">
                <button 
                  onClick={generateNewPasscode}
                  className="bg-amber-500 hover:bg-amber-400 text-black font-bold px-4 py-2.5 rounded-lg text-xs"
                >
                  Generate New Passcode
                </button>
                {newGeneratedPasscode && (
                  <div className="bg-zinc-950 border border-amber-500/60 px-4 py-2 rounded-lg text-amber-300 font-mono font-bold tracking-widest text-sm">
                    {newGeneratedPasscode}
                  </div>
                )}
              </div>
            ...
            {/* Live Price & Package Editor */}
            <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl shadow-xl">
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h3 className="text-xl font-bold text-amber-400">💰 Edit Price Packages (Syncs to Price.jsx)</h3>
                  <p className="text-xs text-zinc-400 mt-1">ኣብዚ ዝተቐየረ ዋጋን መግለጫን ብቐጥታ ናብቲ የዕሩኽ 顾客 Price  ገጽ ይሓልፍ።</p>
                </div>
                <button 
                  onClick={handleSavePricesToBackend}
                  className="bg-green-600 hover:bg-green-700 text-white font-bold px-5 py-2.5 rounded-xl text-xs"
                >
                  Save All Prices
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {pricePackages.map((pkg, idx) => (
                  <div key={pkg.id} className="bg-zinc-950 p-5 rounded-xl border border-zinc-800 space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-xs uppercase tracking-widest text-amber-400 font-bold">{pkg.subtitle}</span>
                      <input 
                        type="text" 
                        value={pkg.name}
                        onChange={(e) => {
                          const updated = [...pricePackages];
                          updated[idx].name = e.target.value;
                          setPricePackages(updated);
                        }}
                        className="bg-zinc-900 border border-zinc-700 px-2 py-1 rounded text-xs text-white font-bold w-28 text-right"
                      />
                    </div>

                    <div>
                      <label className="block text-[11px] text-zinc-400 mb-1">Price (ዋጋ):</label>
                      <input 
                        type="text" 
                        value={pkg.price}
                        onChange={(e) => {
                        ...
                        className="bg-zinc-900 border border-zinc-700 p-2 rounded w-full text-sm text-amber-400 font-bold"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        className="bg-zinc-900 border border-zinc-700 p-2 rounded w-full text-xs text-zinc-300"
                      />
                    </div>
                  </div>
                ))}
