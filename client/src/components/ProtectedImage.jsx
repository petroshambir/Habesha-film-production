...
const ProtectedImage = ({ src, alt, className }) => {
  return (
    <div 
      className={`relative overflow-hidden select-none group ${className || ''}`}
      onContextMenu={(e) => e.preventDefault()} // የማናይ ጸወታ ይኽልክል
      onDragStart={(e) => e.preventDefault()}    // ብማውስ/ኢድ ሰሒብካ ንምውጻእ ይኽልክል
    >
      {/* 1. እቲ ትክክለኛ ምስሊ */}
      <img 
        src={src} 
      ...
      {/* 2. መከላኸሊ ባዶ ምስሊ (Transparent Layer - Save Image ክገብሩ ከለዉ እዚ እዩ ዝወርድ) */}
      <img 
        src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=" 
        alt="Protection Layer" 
        className="absolute inset-0 w-full h-full opacity-0 z-10"
      ...
      {/* 3. ዋተርማርክ - ኣብ ሞባይልን ፒሲን ብንጹር ንምርኣይ
          - ብንቡር ግዜ (opacity-30) ኮይኑ ብቐዋሚ ክረአ ይገብር (ንሞባይል ኣገዳሲ)
          - ማውስ ክትትንክብ ወይ ክትከውን ከሎ ድማ (group-hover:opacity-60) ይበርህ */}
...
protectedImageExport: export default ProtectedImage;